		...
		if(is_filling) {
			...
			if(is_call(inst) || is_jump(inst)) {
				...
				for(unsigned i=0;i<inst.operand_count();++i) {
					const auto& oper=inst.operands()[i];
					...
					if((oper.complete_type()==edb::Operand::TYPE_IMMEDIATE32 && edb::v1::debuggeeIs32Bit()) ||
							(oper.complete_type()==edb::Operand::TYPE_IMMEDIATE64 && edb::v1::debuggeeIs64Bit()))
					{
					...
					else if(oper.general_type()==edb::Operand::TYPE_EXPRESSION &&
							(oper.expression().base==edb::Operand::Register::X86_REG_INVALID ||
							 oper.expression().base==edb::Operand::Register::X86_REG_RIP) &&
							oper.expression().index==edb::Operand::Register::X86_REG_INVALID &&
							(oper.expression().segment==edb::Operand::Segment::Reg::REG_INVALID ||
							 oper.expression().segment==edb::Operand::Segment::Reg::DS))
					{
						const edb::address_t target = (oper.expression().base==edb::Operand::Register::X86_REG_RIP ?
													   oper.expression().base+inst.size() : 0)
													   + oper.expression().displacement;
						const bool prefixed=showLocalModuleNames || !targetIsLocal(target,inst.rva());
						QString sym = edb::v1::symbol_manager().find_address_name(target,prefixed);
						if(!sym.isEmpty())
						{
